...
switch ($action) {
    case 'create_equipment':
        $stmt = $pdo->prepare("INSERT INTO equipment (asset_tag, name, type, status, assigned_project, assigned_crew_id, last_service_date, next_service_date, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))");
        $stmt->execute([
            $_POST['asset_tag'],
            $_POST['name'],
            $_POST['type'],
    ...
    case 'retire_equipment':
        $pdo->prepare("UPDATE equipment SET status='retired' WHERE id=?")->execute([$_POST['id']]);
    ...
    case 'add_service_log':
        ...
        $pdo->prepare("UPDATE equipment SET last_service_date=? WHERE id=?")->execute([$_POST['service_date'], $_POST['equipment_id']]);
